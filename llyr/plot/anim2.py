...
class anim2(Base):
    ...
    def get_quiver_data(self, arr):
        stepx = max(int(arr.shape[2] / 60), 1)
        stepy = max(int(arr.shape[1] / 60), 1)
        scale = 1 / max(stepx, stepy)
        x, y = np.meshgrid(
            np.arange(0, arr.shape[2], stepx) * self.m.dx * 1e9,
            np.arange(0, arr.shape[1], stepy) * self.m.dy * 1e9,
        )
        u, v, z = (
            arr[:, ::stepy, ::stepx, 0],
            arr[:, ::stepy, ::stepx, 1],
    ...
    def plot(self, mult=0.9):
        arr = self.m.m[:60, 0] - self.m.stable[0, 0] * mult
        ...
        # gs.update()
        arr = np.ma.masked_equal(arr, 0)
        extent = [0, arr.shape[2] * self.m.dx * 1e9, 0, arr.shape[1] * self.m.dy * 1e9]
        ...
        x, y, u, v, alpha, scale = self.get_quiver_data(arr)
        Q = ax.quiver(
            x, y, u[0], v[0], alpha=alpha[0], angles="xy", scale_units="xy", scale=scale
        )
        ts = np.array(self.m.m.attrs["t"][:]) * 1e12
        ts -= ts[0]
        ...
        ani.save(
            f"jobs/anim/{self.m.sim_name}_{mult}.gif", writer="ffmpeg", fps=25, dpi=150
        )
        plt.close()
