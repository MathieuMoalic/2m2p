...
def iplotp(op, path, xstep=2, comps=None, fmin=0, fmax=20):
    ...
    ttext = ax1.text(
        0.7,
        0.08,
        f"Peak threshold = {threshold.val*100:.2f}%",
    ...
    def onclick(event):
        if event.inaxes == ax1:
            ...
            if event.button.name == "RIGHT":
                ...
                peaks = freqs[peakutils.indexes(fft, thres=threshold.val, min_dist=5)]
    ...
    def onpress(event):
        if event.key == "-":
            threshold.val *= 0.8
            ttext.set_text(f"Peak threshold = {threshold.val*100:.2f}%")
        if event.key == "=":
            threshold.val *= 1.2
            ttext.set_text(f"Peak threshold = {threshold.val*100:.2f}%")
        if event.key == "a":
