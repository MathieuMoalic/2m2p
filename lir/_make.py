...
class Make:
    def _load_ovf(self, ovf_path: str) -> np.ndarray:
        ...
        with open(ovf_path, "rb") as f:
            for _ in range(28):
                next(f)
            try:
                arr: np.ndarray = np.fromfile(f, "<f4", count=self._count)[1:].reshape(
                    self._ovf_shape
                )
    ...
    def parse_script(self) -> None:
        ...
        with h5py.File(self.h5_path, "a") as f:
            ...
            def get_attribute(
                key_re: str, value_re: str, region_re: str = None
            ) -> None:
                ...
                if region_re is not None:
                    ...
                    if (
                        isinstance(key_match, Match)
                        and isinstance(value_match, Match)
                        and isinstance(region_match, Match)
                    ):
                        key = (
                            region_match.group().split("_")[0]
                            + "_"
                            + key_match.group().replace(":=", "").replace(" ", "")
                        )
        ...
        with h5py.File(self.h5_path, "a") as f:
            ...
            with mp.Pool(processes=int(mp.cpu_count())) as p:
                for i, data in enumerate(
                    tqdm(
                        p.imap(self._load_ovf, ovf_paths),
                        leave=False,
                        desc=name,
                        total=len(ovf_paths),
                    )
